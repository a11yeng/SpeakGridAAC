...
const PROFILES_STORAGE_KEY = 'speakgrid-aac-profiles-fg-aac-v1';
...
const defaultButtons = [
  ...
  { label: 'Pain', spoken: 'Pain', symbol: '😣', color: '#ffeef5', action: 'speak' },
  { label: 'Stop', spoken: 'Stop', symbol: '🛑', color: '#ffe8e8', action: 'speak' },
  { label: 'Food', spoken: 'Food', symbol: '🍽️', color: '#fff1df', action: 'speakFolder' },
  { label: 'Bathroom', spoken: 'Bathroom', symbol: '🚽', color: '#e7f1ff', action: 'speakFolder' },
  { label: 'Activities', spoken: 'Activities', symbol: '📱', color: '#f1edff', action: 'speakFolder' },
  { label: 'Keyboard', spoken: 'Keyboard', symbol: '⌨️', color: '#eef0ff', action: 'speakFolder' }
];
...
const sampleFolderPages = {
  Food: {
    ...
    rows: 3,
    buttons: [
      { label: 'Pouch', spoken: 'Pouch', symbol: '🥣', color: '#fff1df', action: 'speak' },
      { label: 'Water', spoken: 'Water', symbol: '💧', color: '#e7f7ff', action: 'speak' },
      { label: 'Ice', spoken: 'Ice', symbol: '🧊', color: '#eef7ff', action: 'speak' }
    ]
  },
  Bathroom: {
    columns: 3,
    rows: 3,
    buttons: [
      { label: 'Pee', spoken: 'Pee', symbol: '🟨', color: '#fff8cf', action: 'speak' },
      { label: 'Poop', spoken: 'Poop', symbol: '🟫', color: '#f3e5d8', action: 'speak' }
    ]
  },
  Activities: {
    columns: 3,
    rows: 3,
    buttons: [
      { label: 'TV', spoken: 'TV', symbol: '📺', color: '#eef0ff', action: 'speak' },
      { label: 'Tablet', spoken: 'Tablet', symbol: '📱', color: '#f1edff', action: 'speak' }
    ]
  },
  Keyboard: {
    columns: 10,
    rows: 3,
    buttons: [
      { label: 'Q', spoken: 'Q', symbol: 'Q', color: '#ffffff', action: 'speak' },
      { label: 'W', spoken: 'W', symbol: 'W', color: '#ffffff', action: 'speak' },
      { label: 'E', spoken: 'E', symbol: 'E', color: '#ffffff', action: 'speak' },
      { label: 'R', spoken: 'R', symbol: 'R', color: '#ffffff', action: 'speak' },
      { label: 'T', spoken: 'T', symbol: 'T', color: '#ffffff', action: 'speak' },
      { label: 'Y', spoken: 'Y', symbol: 'Y', color: '#ffffff', action: 'speak' },
      { label: 'U', spoken: 'U', symbol: 'U', color: '#ffffff', action: 'speak' },
      { label: 'I', spoken: 'I', symbol: 'I', color: '#ffffff', action: 'speak' },
      { label: 'O', spoken: 'O', symbol: 'O', color: '#ffffff', action: 'speak' },
      { label: 'P', spoken: 'P', symbol: 'P', color: '#ffffff', action: 'speak' },
      { label: 'A', spoken: 'A', symbol: 'A', color: '#ffffff', action: 'speak' },
      { label: 'S', spoken: 'S', symbol: 'S', color: '#ffffff', action: 'speak' },
      { label: 'D', spoken: 'D', symbol: 'D', color: '#ffffff', action: 'speak' },
      { label: 'F', spoken: 'F', symbol: 'F', color: '#ffffff', action: 'speak' },
      { label: 'G', spoken: 'G', symbol: 'G', color: '#ffffff', action: 'speak' },
      { label: 'H', spoken: 'H', symbol: 'H', color: '#ffffff', action: 'speak' },
      { label: 'J', spoken: 'J', symbol: 'J', color: '#ffffff', action: 'speak' },
      { label: 'K', spoken: 'K', symbol: 'K', color: '#ffffff', action: 'speak' },
      { label: 'L', spoken: 'L', symbol: 'L', color: '#ffffff', action: 'speak' },
      { label: 'HOME', spoken: 'Home', symbol: '🏠', color: '#eff8ed', action: 'speakFolder' },
      { label: 'Z', spoken: 'Z', symbol: 'Z', color: '#ffffff', action: 'speak' },
      { label: 'X', spoken: 'X', symbol: 'X', color: '#ffffff', action: 'speak' },
      { label: 'C', spoken: 'C', symbol: 'C', color: '#ffffff', action: 'speak' },
      { label: 'V', spoken: 'V', symbol: 'V', color: '#ffffff', action: 'speak' },
      { label: 'B', spoken: 'B', symbol: 'B', color: '#ffffff', action: 'speak' },
      { label: 'N', spoken: 'N', symbol: 'N', color: '#ffffff', action: 'speak' },
      { label: 'M', spoken: 'M', symbol: 'M', color: '#ffffff', action: 'speak' },
      { label: 'SOMETHING ELSE', spoken: 'Something else', symbol: '❓', color: '#fff8cf', action: 'speak' }
    ]
  }
};
...
function linkFolderButtons(pages, homePage) {
  const pageByName = Object.fromEntries(pages.map(page => [page.name, page]));
  pages.forEach(page => {
    page.buttons.forEach(button => {
      if (button.action !== 'folder' && button.action !== 'speakFolder') return;
      const targetName = button.label === 'HOME' ? 'Home' : button.label;
      const targetPage = pageByName[targetName] || homePage;
      button.targetPageId = targetPage.id;
    });
  });
}

function defaultBoard() {
  const homePage = makePage('Home', defaultButtons, { columns: 3, rows: 3 });
  const pages = [homePage];

  Object.entries(sampleFolderPages).forEach(([folderName, sample]) => {
    const childPage = makePage(folderName, sample.buttons || [], {
  ...
  linkFolderButtons(pages, homePage);
...
function syncControlsToState() {
  ...
  setupSelect(columnsSelect, 1, 10, pageColumns(page));
